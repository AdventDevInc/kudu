...
export function createWin32Security(): PlatformSecurity {
  return {
    ...
    async collectAntivirusStatus(): Promise<HealthReport['securityPosture']['antivirus']> {
      ...
      const items: Array<{ displayName: string; productState: number }> = Array.isArray(raw)
        ? raw
        : [raw]
      ...
      const primary =
        thirdParty[0]?.name ?? products.find((p) => p.enabled && p.realTimeProtection)?.name ?? null
    ...
    async collectFirewallStatus(): Promise<HealthReport['securityPosture']['firewall']> {
      ...
      if (fwProfiles.status === 'fulfilled') {
        try {
          ...
          const profiles: Array<{ Name: string; Enabled: number | boolean }> = Array.isArray(raw)
            ? raw
            : [raw]
          const lookup = Object.fromEntries(
            profiles.map((p) => [p.Name?.toLowerCase(), !!p.Enabled])
          )
      ...
      const windowsEnabled =
        windowsProfiles.domain && windowsProfiles.private && windowsProfiles.public
    ...
    async collectDiskEncryptionStatus(): Promise<HealthReport['securityPosture']['bitlocker']> {
      try {
        ...
        const statusMap: Record<
          number,
          HealthReport['securityPosture']['bitlocker']['volumes'][0]['status']
        > = {
          0: 'FullyDecrypted',
          1: 'FullyEncrypted',
    ...
    async collectUpdateStatus(): Promise<HealthReport['securityPosture']['windowsUpdate']> {
      ...
      const recentPatches = patches
        ...
        .map((p) => {
          ...
          return {
            id: p.HotFixID,
            installedOn: isNaN(date.getTime())
              ? String(installedRaw ?? '')
    ...
    async collectScreenLockStatus(): Promise<HealthReport['securityPosture']['screenLock']> {
      ...
      const inactivityLockSec =
        typeof data.gpoTimeout === 'number' && data.gpoTimeout > 0 ? data.gpoTimeout : null
