...
async function gsettingsGet(schema: string, key: string): Promise<string> {
  const { stdout } = await execFileAsync('/usr/bin/gsettings', ['get', schema, key], {
    timeout: 5_000
  })
  return stdout.trim()
}
...
async function sysctlRevert(param: string, softValue: string): Promise<void> {
  ...
  await execFileAsync('/usr/sbin/sysctl', ['-w', `${param}=${softValue}`], {
    timeout: 5_000
  }).catch(() => {})
  let existing = ''
  try {
...
const KDE_PRIVACY_SETTINGS: PrivacySettingDef[] = [
  ...
  },
  {
    ...
    async check() {
      try {
        const val = await kdeConfigRead(
          'kactivitymanagerdrc',
          'Plugins',
          'org.kde.ActivityManager.ResourceScoringEnabled'
        )
        return val === 'false'
      } catch {
        return false
