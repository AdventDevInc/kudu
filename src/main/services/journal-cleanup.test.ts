...
vi.mock('fs/promises', () => ({
  lstat: vi.fn(async (path: string) => ({
    isDirectory: () => true,
    isSymbolicLink: () => state.link === path
  }))
}))
...
async function scan() {
  return scanManagedCleanup('journal-vacuum', 'system', 'Journal Logs', JOURNAL_ROOT)
}
beforeEach(() => {
  ...
  vi.mocked(execTracked)
    .mockReset()
    .mockResolvedValue({ stdout: 'Archived and active journals take up 48.0M.', stderr: '' })
})
...
describe('native journal cleanup', () => {
  it('offers unselected maintenance using read-only discovery and unknown savings', async () => {
    ...
    expect(result.items[0]).toMatchObject({
      ...
      size: 0,
      selected: false,
      cleanupAction: 'journal-vacuum'
    })
    expect(execTracked).toHaveBeenCalledExactlyOnceWith(
      '/usr/bin/journalctl',
      ['--no-pager', '--directory=/var/log/journal', '--disk-usage'],
      { timeout: 10_000 }
    )
  })
  it('vacuuming is confined to persistent archives with bounded retention and no rotation', async () => {
    ...
    expect(execTracked).toHaveBeenLastCalledWith(
      '/usr/bin/journalctl',
      ['--no-pager', '--directory=/var/log/journal', '--vacuum-time=30d', '--vacuum-size=1G'],
      { timeout: 300_000 }
    )
  })
  ...
  it.each(['/var', '/var/log', JOURNAL_ROOT])(
    'rejects a journal ancestor replaced by a link: %s',
    async (path) => {
      const item = (await scan()).items[0]
      state.link = path
      vi.mocked(execTracked).mockClear()
  ...
  it('does not offer missing journalctl or fall back to deleting files on failure', async () => {
    ...
    expect((await scan()).items).toEqual([])
    expect(await runManagedCleanup(item)).toEqual({
      success: false,
