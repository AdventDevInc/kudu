...
async function scan() {
  return scanManagedCleanup('docker-build-prune', 'app', 'Docker Build Cache', '/unused')
}
beforeEach(() => {
  state.context = 'desktop-linux'
  state.endpoint = 'npipe:////./pipe/dockerDesktopLinuxEngine'
  state.daemon = 'engine-1'
  state.exclusions = []
  vi.mocked(execTracked)
    .mockReset()
    .mockImplementation(async (_file, args) => {
      ...
      if (args[0] === 'context' && args[1] === 'inspect')
        return {
          stdout: JSON.stringify([{ Endpoints: { docker: { Host: state.endpoint } } }]),
          stderr: ''
        }
...
describe('Docker build cache maintenance', () => {
  it('previews the local engine and retention policy without pruning or selecting it', async () => {
    ...
    expect(result.items[0]).toMatchObject({
      selected: false,
      size: 0,
      dockerTarget: { context: state.context, endpoint: state.endpoint, daemonId: state.daemon }
    })
  ...
  it('pins pruning to the scanned local socket and bounds cache removal', async () => {
    ...
    expect(execTracked).toHaveBeenLastCalledWith(
      'docker',
      [
        '--host',
        state.endpoint,
  ...
  it.each([
    ...
    'unix://remote/socket'
  ])('withholds unsupported or remote endpoint %s', async (endpoint) => {
    state.endpoint = endpoint
    expect((await scan()).items).toEqual([])
    expect(
      vi
        .mocked(execTracked)
        .mock.calls.some(([, args]) => args.includes('info') || args.includes('prune'))
    ).toBe(false)
  })
  it.each(['context', 'endpoint', 'daemon'] as const)(
    'requires rescanning when %s changes',
    async (field) => {
      const item = (await scan()).items[0]
      state[field] = field === 'endpoint' ? 'unix:///var/run/docker.sock' : 'changed'
      expect((await runManagedCleanup(item)).success).toBe(false)
      expect(vi.mocked(execTracked).mock.calls.some(([, args]) => args.includes('prune'))).toBe(
        false
      )
    }
  )
