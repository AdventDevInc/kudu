...
const windowsAction = (action: ManagedCleanupAction) =>
  action === 'windows-components' || action === 'delivery-optimization'
...
async function packageCommand(action: 'uv-prune' | 'pnpm-prune', prune: boolean) {
  if (action === 'uv-prune')
    return execTracked('uv', ['cache', prune ? 'prune' : 'dir'], {
      timeout: prune ? 300_000 : 10_000,
      cwd: homedir()
    })
  ...
  if (process.platform === 'win32') {
    return execTracked(
      systemExe('cmd.exe'),
      ['/d', '/s', '/c', prune ? 'pnpm store prune' : 'pnpm store path'],
      { timeout: prune ? 300_000 : 10_000, cwd: homedir() }
    )
  }
  return execTracked('pnpm', args, { timeout: prune ? 300_000 : 10_000, cwd: homedir() })
}

export async function scanManagedCleanup(
  action: ManagedCleanupAction,
  category: string,
  label: string,
  fallbackPath: string
): Promise<ScanResult> {
      ...
      if (action === 'delivery-optimization') {
        await execTracked(
          ...
          [
            ...
            '-NonInteractive',
            '-Command',
            "$ErrorActionPreference='Stop'; Get-Command Delete-DeliveryOptimizationCache -ErrorAction Stop | Out-Null"
          ],
          { timeout: 10_000 }
        )
...
export async function runManagedCleanup(
  item: ScanItem
): Promise<{ success: boolean; reason?: string }> {
  const action = item.cleanupAction
  if (!action || !actions.has(action)) return { success: false, reason: 'unsupported-cleanup' }
  if ((getSettings().exclusions ?? []).length)
    return {
      success: false,
      reason: 'Native cleanup cannot honor file exclusions; use ordinary file cleanup instead.'
    }
  if (running.has(action)) return { success: false, reason: 'in-use' }
  if (windowsAction(action) && (process.platform !== 'win32' || !isAdmin()))
    return { success: false, reason: 'permission-denied' }
  if (action === 'journal-vacuum' && (process.platform !== 'linux' || !isAdmin()))
    return { success: false, reason: 'permission-denied' }
  running.add(action)
  try {
    if (action === 'journal-vacuum') {
      ...
      if (!isAbsolute(current) || resolve(current) !== resolve(item.path))
        return { success: false, reason: 'Cache location changed; scan again.' }
      ...
      await execTracked(
        ...
        [
          ...
          '-NonInteractive',
          '-Command',
          "$ErrorActionPreference='Stop'; Delete-DeliveryOptimizationCache -Force -ErrorAction Stop"
        ],
        { timeout: 300_000 }
      )
    }
    ...
    return {
      ...
      reason: err.killed
        ? 'Cleanup timed out.'
        : String(err.message || 'Native cleanup failed.').slice(0, 500)
    }
  } finally {
