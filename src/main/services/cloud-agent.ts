...
import {
  listStartupItems as listStartupItemsWin32,
  toggleStartupItem as toggleStartupItemWin32
} from '../ipc/startup-manager.ipc'
...
function sanitizeOsRelease(release: string | undefined, platform = process.platform): string {
  ...
  if (!release || /^\d+\.\d+\.\d+-.+/.test(release)) {
    try {
      ...
      /* file missing — e.g. minimal container */
    }
  }
  return release ?? ''
}
...
class CloudAgentService {
    ...
    const vulnerabilities = rawItems
      ...
      .map((item) => {
        ...
        return {
          ...
          installedVersion:
            typeof item.installed_version === 'string' ? item.installed_version : '',
          severity: (typeof item.severity === 'string' && validSeverities.has(item.severity)
            ? item.severity
            : 'none') as import('../../shared/types').CveSeverity,
  ...
  async addBreachMonitorEmails(
    emails: string[]
  ): Promise<import('../../shared/types').BreachMonitorResult> {
    if (this.status !== 'connected') throw new Error('Cloud agent not connected')
    const path = `/devices/${encodeURIComponent(this.deviceId)}/breach-monitor`
  ...
  async acknowledgeBreaches(
    breachIds: string[]
  ): Promise<import('../../shared/types').BreachAcknowledgeResult> {
    if (this.status !== 'connected') throw new Error('Cloud agent not connected')
  ...
  private parseBreachMonitorResponse(
    raw: Record<string, unknown>
  ): import('../../shared/types').BreachMonitorResult {
    ...
    const emails = rawEmails
      ...
      .map((item) => {
        ...
        return {
          ...
          monitoringPaused:
            typeof item.monitoring_paused === 'boolean' ? item.monitoring_paused : false,
          breaches
        }
      })
  ...
  async link(apiKey: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const friendly =
        err instanceof CloudHttpError && err.isTerminal ? this.formatTerminalError(err) : msg
  ...
  private async discover(): Promise<void> {
    ...
    cloudLog('INFO', 'Discovery complete', {
      wsHost: data.ws.host,
      wsPort: data.ws.port,
      tls: data.ws.tls,
      api: data.api,
  ...
  private needsPayloadFetch(cmd: CloudCommand): boolean {
    switch (cmd.type) {
      ...
      case 'driver-update-install':
        return !cmd.updateIds
      case 'driver-clean':
        return !cmd.publishedNames
      case 'privacy-apply':
        return !cmd.settingIds
      case 'debloater-remove':
        return !cmd.packageNames
      case 'service-apply':
        return !cmd.changes
  ...
  private onCommand(raw: unknown): void {
    ...
    if (!('requestId' in cmd) || typeof cmd.requestId !== 'string' || cmd.requestId.length > 200)
      return
  ...
  private startWatchdog(): void {
    ...
    this.watchdogTimer = setInterval(() => {
      ...
      if (elapsed > WATCHDOG_STALE_MS) {
        cloudLog(
          'ERROR',
          `Watchdog: no successful telemetry for ${Math.round(elapsed / 1000)}s — forcing reconnect`
        )
        this.forceReconnect()
      }
    }, WATCHDOG_INTERVAL)
  ...
  private async collectAndSendTelemetry(): Promise<void> {
    ...
    try {
      ...
      const fastCalls = Promise.all([si.currentLoad(), si.mem(), si.disksIO(), si.time()])
      ...
      cloudLog(
        'DEBUG',
        `Telemetry sent (tick ${this.telemetryTick}, cpu=${snapshot.cpu.toFixed(1)}%, mem=${snapshot.memoryPercent.toFixed(1)}%)`
      )
    } catch (err) {
  ...
  private async collectAndSendHealthReport(): Promise<void> {
    ...
    try {
      ...
      const [r0, r1, r2] = await withTimeout(
        Promise.allSettled([
  ...
  private async collectSecurityPosture(): Promise<HealthReport['securityPosture']> {
    ...
    return {
      ...
      windowsUpdate:
        wu.status === 'fulfilled'
          ? wu.value
          : { recentPatches: [], lastPatchDate: null, daysSinceLastPatch: null },
      screenLock:
        sl.status === 'fulfilled'
          ? sl.value
          : {
  ...
  private startThreatMonitor(): void {
    ...
    threatMonitor.setThreatCallback((snapshot) => {
      ...
      if (!isDaemon && currentSettings.showThreatNotifications && Notification.isSupported()) {
        ...
        const parts: string[] = []
        if (connCount > 0)
          parts.push(`${connCount} suspicious connection${connCount > 1 ? 's' : ''}`)
  ...
  private async fetchInitialBlacklist(): Promise<void> {
    try {
      ...
      if (result.success) {
        ...
        cloudLog(
          'INFO',
          `Initial blacklist loaded (${result.stats!.domains} domains, ${result.stats!.ips} IPs, ${result.stats!.cidrs} CIDRs)`
        )
      } else {
  ...
  private queueThreatAlert(snapshot: ThreatSnapshot): void {
    ...
    if (this.threatAlertPending) {
      if (
        this.threatAlertPending.flaggedConnections.length < CloudAgentService.THREAT_PENDING_MAX
      ) {
  ...
  private flushThreatAlert(): void {
    ...
    if (this.threatAlertHourlyCount >= CloudAgentService.THREAT_ALERT_MAX_PER_HOUR) {
      cloudLog(
        'DEBUG',
        `Threat alert suppressed (hourly cap ${CloudAgentService.THREAT_ALERT_MAX_PER_HOUR} reached)`
      )
      return
    }
  ...
  private stopThreatMonitor(): void {
    ...
    threatMonitor.stop()
    if (this.threatAlertCooldownTimer) {
      clearTimeout(this.threatAlertCooldownTimer)
      this.threatAlertCooldownTimer = null
    }
    this.threatAlertPending = null
  }
  ...
  private async postThreatAlert(snapshot: ThreatSnapshot): Promise<void> {
    ...
    cloudLog(
      'INFO',
      `Threat alert sent (${snapshot.flaggedConnections.length} connections, ${snapshot.flaggedDns.length} DNS)`
    )
  }
  ...
  private checkCommandPermission(type: string): string | null {
    ...
    switch (type) {
      case 'shutdown':
      case 'restart':
        if (s.allowRemotePower === false)
          return 'Disabled by user: remote power control is turned off'
        break
      case 'clean':
      case 'malware-quarantine':
      case 'malware-delete':
      ...
      case 'run-sfc':
      case 'run-dism':
        if (s.allowRemoteInstalls === false)
          return 'Disabled by user: remote installs are turned off'
        break
      case 'startup-toggle':
      case 'privacy-apply':
      case 'service-apply':
        if (s.allowRemoteConfig === false)
          return 'Disabled by user: remote config changes are turned off'
        break
    }
    return null
  }
    ...
    if (!isParallelSafe && this.commandRunning) {
      if ('requestId' in cmd) {
        this.postCommandResult(
          ...
          false,
          undefined,
          'A mutating command is already running'
        ).catch(() => {})
      }
      return
    }
    ...
    if (!isParallelSafe && this.runningCommands > 0) {
      if ('requestId' in cmd) {
        this.postCommandResult(
          ...
          false,
          undefined,
          'Commands are still running — try again shortly'
        ).catch(() => {})
      }
      return
    }
    ...
    if (isParallelSafe && this.runningCommands >= MAX_PARALLEL_COMMANDS) {
      if ('requestId' in cmd) {
        this.postCommandResult(
          ...
          false,
          undefined,
          'Too many commands running — try again shortly'
        ).catch(() => {})
      }
      return
    }
    ...
    try {
      ...
      switch (cmd.type) {
        ...
        case 'driver-clean':
          ...
          else if (cmd.type === 'driver-update-install')
        ...
        case 'startup-list':
          ...
          break
        case 'startup-toggle':
          await this.handleStartupToggle(
            cmd.requestId,
            cmd.name,
            cmd.location,
            cmd.command,
  ...
  private getCommandSummary(cmd: CloudCommand): string {
    switch (cmd.type) {
      case 'scan':
        return `Scan: ${cmd.scanType}`
      case 'clean':
        return `Clean: ${cmd.itemIds?.length ?? 0} items`
      case 'software-update-check':
        return 'Check software updates'
      case 'software-update-run':
        return `Update ${cmd.appIds?.length ?? 0} apps`
      case 'get-status':
        return 'Get device status'
      case 'get-system-info':
        return 'Get system info'
      case 'get-health-report':
        return 'Generate health report'
      case 'ping':
        return 'Ping'
      case 'shutdown':
        return `Shutdown${cmd.delaySec ? ` (${cmd.delaySec}s delay)` : ''}`
      case 'restart':
        return `Restart${cmd.delaySec ? ` (${cmd.delaySec}s delay)` : ''}`
      case 'windows-update-check':
        return 'Check Windows updates'
      case 'windows-update-install':
        return 'Install Windows updates'
      case 'run-sfc':
        return process.platform === 'linux' ? 'Clean package cache' : 'Run System File Checker'
      case 'run-dism':
        return process.platform === 'linux' ? 'Remove orphaned packages' : 'Run DISM repair'
      case 'get-network-config':
        return 'Get network config'
      case 'get-event-log':
        return `Get event log: ${cmd.logName ?? 'System'}`
      case 'get-installed-apps':
        return 'Get installed apps'
      case 'driver-update-scan':
        return 'Scan driver updates'
      case 'driver-update-install':
        return `Install ${cmd.updateIds?.length ?? 0} driver updates`
      case 'driver-clean':
        return `Clean ${cmd.publishedNames?.length ?? 0} drivers`
      case 'startup-list':
        return 'List startup items'
      case 'startup-toggle':
        return `Toggle startup: ${cmd.name}`
      case 'disk-health':
        return 'Check disk health'
      case 'privacy-scan':
        return 'Privacy scan'
      case 'privacy-apply':
        return `Apply ${cmd.settingIds?.length ?? 0} privacy settings`
      case 'debloater-scan':
        return 'Debloater scan'
      case 'debloater-remove':
        return `Remove ${cmd.packageNames?.length ?? 0} packages`
      case 'service-scan':
        return 'Service scan'
      case 'service-apply':
        return `Apply ${cmd.changes?.length ?? 0} service changes`
      case 'malware-quarantine':
        return `Quarantine ${cmd.paths?.length ?? 0} threats`
      case 'malware-delete':
        return `Delete ${cmd.paths?.length ?? 0} threats`
      case 'registry-scan':
        return 'Registry scan'
      case 'registry-fix':
        return `Fix ${cmd.entryIds?.length ?? 0} registry entries`
      case 'update-threat-blacklist':
        return 'Update threat blacklist'
      case 'update-yara-rules':
        return 'Update YARA rules'
      case 'get-threat-status':
        return 'Get threat status'
      case 'cve-scan':
        return 'CVE vulnerability scan'
      default:
        return (cmd as CloudCommand).type
    }
  }
    ...
    switch (scanType) {
        ...
        for (const t of targets) {
          try {
            ...
            if (t.cleanupAction || t.cacheReset) {
              ...
              r = await scanMultipleDirectories(
                childPaths,
                CleanerType.System,
                t.subcategory,
                recency
      ...
      case 'gaming': {
        ...
        for (const launcher of getPlatform().paths.gamingPaths()) {
          ...
          try {
            const r = await scanAppRule(launcher, gamingCategory, {
              directoryItems: true,
        ...
        for (const gpu of getPlatform().paths.gpuCachePaths()) {
          ...
          try {
            const r = await scanAppRule(gpu, gamingCategory, {
              directoryItems: true,
      ...
      default:
        await this.postCommandResult(
          ...
          false,
          undefined,
          'Scan type not yet supported via cloud'
        )
        return
    }
  }
  ...
  private async handleClean(requestId: string, itemIds: string[]): Promise<void> {
    ...
    const fileResult =
      fileIds.length > 0
        ? await cleanItems(fileIds, undefined, 'cloud')
        : {
            totalCleaned: 0,
            filesDeleted: 0,
            filesSkipped: 0,
    ...
    const trashPath = getPlatform().paths.trashPath()
    if (trashPath && cleansTrash) {
      const { pruneOrphanedTrashInfo } = await import('./trash-info')
  ...
  private async handleGetSystemInfo(requestId: string): Promise<void> {
    ...
    await this.postCommandResult(requestId, true, {
      ...
      os: {
        ...
        release: sanitizeOsRelease(osInfo.release),
  ...
  private async handleGetEventLog(
    requestId: string,
    logName?: string,
    maxEntries?: number
  ): Promise<void> {
  ...
  private async handleDriverClean(requestId: string, publishedNames: string[]): Promise<void> {
    if (
      !Array.isArray(publishedNames) ||
      publishedNames.length === 0 ||
      publishedNames.length > 100
    ) {
  ...
  private async handleStartupToggle(
    ...
    if (enabled) {
      await this.postCommandResult(
        ...
        false,
        undefined,
        'Remote enable of startup items is not permitted — only disable is allowed via cloud'
      )
      return
    }
    ...
    await this.postCommandResult(
      ...
      success,
      { name, enabled },
      success ? undefined : 'Failed to toggle startup item'
    )
  ...
  private async submitStartupPrograms(): Promise<StartupSafetyResult> {
    ...
    const raw = (await this.postApi(
      `/devices/${encodeURIComponent(this.deviceId)}/startup-programs`,
      {
  ...
  private async submitInstalledPrograms(): Promise<StartupSafetyResult> {
    ...
    const raw = (await this.postApi(
      `/devices/${encodeURIComponent(this.deviceId)}/installed-programs`,
      {
    ...
    cloudLog(
      'DEBUG',
      `installed-programs response: pending=${raw?.pending}, ratings=${Array.isArray(raw?.ratings) ? raw!.ratings.length : 'none'}, keys=${raw ? Object.keys(raw).join(',') : 'null'}`
    )
    const rawItems = Array.isArray(raw?.ratings) ? raw!.ratings : []
    if (rawItems.length > 0) {
      cloudLog(
        'DEBUG',
        `installed-programs first rating sample: ${JSON.stringify(rawItems[0]).slice(0, 200)}`
      )
    }
    const ratings = rawItems
      .filter(
  ...
  private async handleServiceApply(
    requestId: string,
    ...
    if (
      changes.some((c) => typeof c.name !== 'string' || !validStartTypes.has(c.targetStartType))
    ) {
      await this.postCommandResult(
        requestId,
        false,
        undefined,
        'Invalid changes — targetStartType must be Disabled, Manual or Automatic'
      )
      return
    }
  ...
  private async handleMalwareQuarantine(requestId: string, paths: string[]): Promise<void> {
    ...
    if (!paths.every((p) => isAllowedMalwarePath(p))) {
      await this.postCommandResult(
        ...
        false,
        undefined,
        'One or more paths are outside allowed directories'
      )
      return
    }
  ...
  private async handleRegistryScan(requestId: string): Promise<void> {
    ...
    await this.postCommandResult(requestId, true, {
      ...
      byRisk: entries.reduce<Record<string, number>>((acc, e) => {
  ...
  private async handleRegistryFix(requestId: string, entryIds: string[]): Promise<void> {
    ...
    if (entriesToFix.length === 0) {
      await this.postCommandResult(
        ...
        false,
        undefined,
        'No matching entries found — run registry-scan first'
      )
      return
    }
  ...
  private async handleUpdateThreatBlacklist(requestId: string, url: string): Promise<void> {
      ...
      if (app.isPackaged) {
        ...
        if (host === 'localhost' || host === '127.0.0.1' || host === '[::1]' || host === '::1') {
          await this.postCommandResult(
            ...
            false,
            undefined,
            'Private/loopback URLs not allowed'
          )
          return
        }
    ...
    try {
      ...
      await this.postCommandResult(
        ...
        false,
        undefined,
        err instanceof Error ? err.message : 'DNS rebinding check failed'
      )
      return
    }
