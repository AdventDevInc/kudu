...
const roots: string[] = []
async function root() {
  const p = await mkdtemp(join(tmpdir(), 'kudu-measure-'))
  roots.push(p)
  return p
}
afterEach(async () => {
...
function item(path: string, size: number, id = 'one') {
  return { id, path, size, category: 'app', subcategory: 'cache', selected: true, lastModified: 0 }
}
describe('cleanup measurements', () => {
  it('includes deep files but excludes junction targets from sizing', async () => {
    ...
    const deep = join(cache, 'a', 'b', 'c', 'd')
    const outside = join(p, 'outside')
    await mkdir(deep, { recursive: true })
    await mkdir(outside)
    await writeFile(join(deep, 'data'), Buffer.alloc(4096))
    await writeFile(join(outside, 'keep'), Buffer.alloc(8192))
