 ...
 */
async function findMissingClsidDll(
  clsid: string,
  signal?: AbortSignal
): Promise<string | 'no-inproc' | null> {
...
export async function scanRegistry(signal?: AbortSignal): Promise<RegistryEntry[]> {
  ...
  try {
    ...
    for (const block of blocks) {
      ...
      for (const line of block.split(/\r?\n/)) {
        ...
        try {
          await execReg(
            ['query', `HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\${appName}`],
            { timeout: 5000, signal }
          )
        } catch {
          entries.push({
    ...
    try {
      ...
      for (const block of blocks) {
        ...
        if (!installDirExists) {
          ...
          if (iconMatch) {
            const iconPath = expandEnvVars(
              iconMatch[1].trim().replace(/"/g, '').split(',')[0].trim()
            )
            if (
              iconPath &&
              iconPath.includes('\\') &&
  ...
  for (const client of clientCategories) {
    try {
      ...
      const lines = stdout.split(/\r?\n/)
      for (const line of lines) {
        const subKeyMatch = line.match(
          /^(HK\w+\\SOFTWARE\\(?:WOW6432Node\\)?Clients\\[^\\]+\\(.+))$/m
        )
  ...
  try {
    ...
    for (const line of lines) {
      const subKeyMatch = line.match(
        /^(HKLM\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\(.+))$/m
      )
      if (!subKeyMatch) continue
  ...
  try {
    ...
    if (match && match[1] === '0') {
      entries.push({
        ...
        valueName: 'EnableLUA',
        issue:
          'User Account Control (UAC) is disabled — malware can run with admin privileges silently',
        risk: 'high',
        selected: true,
  ...
  try {
    const { stdout } = await execReg(
      ['query', 'HKLM\\SYSTEM\\CurrentControlSet\\Services\\SysMain', '/v', 'Start'],
      { timeout: 5000, signal }
    )
    const match = stdout.match(/Start\s+REG_DWORD\s+0x(\d+)/i)
    if (match && (match[1] === '2' || match[1] === '3')) {
      ...
      try {
        ...
        /* Assume HDD if detection fails — safer to leave SysMain enabled */
      }
      ...
      if (isSSD) {
        ...
        entries.push({
          ...
          valueName: 'Start',
          issue:
            'SysMain (Superfetch) is enabled — improves performance on HDDs, only disable if you have an SSD',
          risk: 'low',
          selected: false,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '4' }
        })
  ...
  try {
    ...
    if (!match || match[1] !== '0') {
      entries.push({
        ...
        issue:
          'LLMNR is enabled — vulnerable to name resolution poisoning attacks on local networks',
        risk: 'medium',
        selected: true,
        fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
      })
  ...
  try {
    ...
    if (!match || match[1] !== '1') {
      entries.push({
        ...
        issue:
          'WPAD auto-proxy discovery is enabled — can be exploited for man-in-the-middle attacks',
        risk: 'medium',
        selected: true,
        fix: { op: 'set-value', regType: 'REG_DWORD', data: '1' }
      })
  ...
  try {
    const { stdout } = await execNativeUtf8('schtasks', ['/query', '/fo', 'CSV', '/nh', '/v'], {
      timeout: 20000,
      signal
    })
    ...
    for (const line of lines) {
      ...
      if (
        !taskToRun ||
        taskToRun === 'N/A' ||
        taskToRun.startsWith('COM handler') ||
        seen.has(taskName)
      )
        continue
      seen.add(taskName)
  ...
  try {
    const { stdout } = await execNativeUtf8('schtasks', ['/query', '/fo', 'CSV', '/v', '/nh'], {
      timeout: 15000,
      signal
    })
 ...
 */
export function collectBackupTargets(entries: RegistryEntry[]): {
  keys: string[]
  tasks: string[]
} {
...
export async function fixRegistryEntries(
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (!entry || !entry.fix) {
      ...
      failures.push({
        issue: 'Unknown entry',
        reason: 'Entry data not found — try scanning again before fixing'
      })
      continue
    }
...
export function registerRegistryCleanerIpc(getWindow: WindowGetter): void {
  ...
  ipcMain.handle(
    ...
    (_event, signatures: string[], ignored: boolean) => {
